...
describe("space datasets", () => {
  it("should export curated planets, moons, stars, and galaxies", () => {
  ...
  it("should look up a planet by name", () => {
    const earth = getPlanetByName("Earth");
    expect(earth?.orderFromSun).toBe(3);
    expect(earth?.type).toBe("terrestrial");
  });

  it("should return moons for a planet", () => {
    const jupiterMoons = getMoonsByPlanet("jupiter");
    expect(jupiterMoons.map((moon) => moon.name)).toContain("Europa");
    expect(jupiterMoons.map((moon) => moon.name)).toContain("Ganymede");
  });

  it("should look up stars and the Milky Way", () => {
    expect(getStarByName("Sirius")?.constellation).toBe("Canis Major");
    expect(getGalaxyByName("Milky Way")?.estimatedStarsMin).toBeGreaterThan(1e11 - 1);
  });
});
...
describe("city capital roles", () => {
  it("should return country capitals", () => {
    const countryCapitals = getCitiesByCapitalType("country");
    expect(countryCapitals.length).toBeGreaterThan(100);
    expect(countryCapitals.some((city) => city.name === "Washington D.C.")).toBe(true);
    expect(countryCapitals.some((city) => city.name === "Tokyo")).toBe(true);
  });

  it("should return subnational capitals where explicitly modeled", () => {
    const stateCapitals = getCitiesByCapitalType("state");
    const provinceCapitals = getCitiesByCapitalType("province");

    expect(stateCapitals.some((city) => city.name === "Sydney")).toBe(true);
    expect(stateCapitals.some((city) => city.name === "Phoenix")).toBe(true);
    expect(stateCapitals.some((city) => city.name === "Berlin")).toBe(true);
    expect(provinceCapitals.some((city) => city.name === "Toronto")).toBe(true);
    expect(provinceCapitals.some((city) => city.name === "Edmonton")).toBe(true);
  });

  it("should detect capital roles on individual city records", () => {
    const washington = cities.find((city) => city.name === "Washington D.C.");
    const sydney = cities.find((city) => city.name === "Sydney");
    const vancouver = cities.find((city) => city.name === "Vancouver");

    expect(washington && isCapitalCity(washington, "country")).toBe(true);
    expect(washington && isCapitalCity(washington, "district")).toBe(true);
    expect(sydney && isCapitalCity(sydney, "state")).toBe(true);
    expect(vancouver && isCapitalCity(vancouver, "province")).toBe(false);
  });
});
...
describe("places", () => {
  it("should expose place dataset metadata", () => {
  ...
  it("should load places by country on demand", async () => {
    const places = await getPlacesByCountry("MT");
    expect(places.some((place) => place.name === "Valletta")).toBe(true);
  });

  it("should search places within a single country", async () => {
    const places = await searchPlacesByCountry("US", "new york");
    expect(places.some((place) => place.name === "New York City")).toBe(true);
    expect(places.every((place) => place.country === "US")).toBe(true);
  });
});
...
describe("getFlagsByColor", () => {
  ...
  it("should require all provided colors by default", () => {
    const redAndOrangeFlags = getFlagsByColor(["red", "orange"]);

    expect(redAndOrangeFlags.length).toBeGreaterThan(1);
    expect(redAndOrangeFlags.some((f) => f.alpha2 === "AM")).toBe(true);
    expect(redAndOrangeFlags.some((f) => f.alpha2 === "ZM")).toBe(true);
    expect(redAndOrangeFlags.some((f) => f.alpha2 === "BT")).toBe(false);
    expect(redAndOrangeFlags.every((f) => f.colors.includes("red") && f.colors.includes("orange"))).toBe(true);
  });

  it("should allow matching any provided color with the or operator", () => {
    const redOrOrangeFlags = getFlagsByColor(["red", "orange"], { operator: "or" });

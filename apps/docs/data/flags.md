...
import type { FlagInfo, FlagColor } from "/data";
```
...
```ts
import { getFlagSvgUrl } from "/data";
...
```ts
import { getFlagPngUrl } from "/data";
...
```ts
import { getCountryMapSvgUrl } from "/data";
...
```ts
import { getSimilarFlags, getFlagData } from "/data";
...
```ts
import { getFlagsByColor, flagData } from "/data";

// All flags with green
const greenFlags = getFlagsByColor("green");

// All flags with both red and orange
...
```ts
import { flagData, getSimilarFlags, getFlagSvgUrl, getCountryByCode } from "/data";

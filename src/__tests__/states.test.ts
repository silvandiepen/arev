...
describe("states", () => {
  ...
  it("should include all 50 US states", () => {
    const usStates = states.filter(
      (s) => s.countryCode === "US" && s.type === "state"
    );
  ...
  it("should have correct country codes (ISO alpha-2)", () => {
    for (const state of states) {
      expect(
        state.countryCode.length,
        `State "${state.name}" has invalid country code: "${state.countryCode}"`
      ).toBe(2);
    }
  });

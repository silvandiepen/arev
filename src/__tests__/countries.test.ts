...
describe("countries", () => {
  ...
  it("should have ISO alpha-2 codes of length 2", () => {
    for (const country of countries) {
      expect(
        country.code.length,
        `Country "${country.name}" has invalid alpha-2 code: "${country.code}"`
      ).toBe(2);
    }
  });
  ...
  it("should have ISO alpha-3 codes of length 3", () => {
    for (const country of countries) {
      expect(
        country.code3.length,
        `Country "${country.name}" has invalid alpha-3 code: "${country.code3}"`
      ).toBe(3);
    }
  });
  ...
  it("should have dial codes starting with +", () => {
    for (const country of countries) {
      expect(
        ...
        `Country "${country.name}" has invalid dial code: "${country.dialCode}"`
      ).toBe(true);
    }
  });
